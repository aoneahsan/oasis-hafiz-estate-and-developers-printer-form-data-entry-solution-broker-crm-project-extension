            ...
            allRQClients?.length > 0 ? (
              <div className='pt-1 overflow-auto mt-7'>
                ...
                <div className='w-[64.4375rem]'>
                  <table className='w-full text-sm text-left text-tertiary'>
                    <thead className='border-b border-[rgba(58,54,83,0.54)]'>
                      <tr className='*:text-tertiary *:py-3 *:text-start *:ps-1 *:text-[1rem] *:font-bold *:font-roboto-bold *:tracking-[0.15px] *:leading-[1.5rem]'>
                    ...
                    <tbody>
                      {zClientsDataRStateSelector?.data?.map((el) => {
                        return (
                          <tr
                            key={el.id}
                            className='*:py-3 border-b border-[rgba(58,54,83,0.54)] *:text-tertiary *:text-start *:ps-2 *:text-[1rem] *:font-normal *:font-roboto-regular *:tracking-[0.15px] *:leading-[1.5rem] *:w-[12.8125rem]'
                          >
                            <td>
                              <div className='overflow-hidden text-ellipsis line-clamp-1'>
                                {isZNonEmptyString(el?.company)
                                  ? el?.company
                                  : constants.noValueFound}
                              </div>
                            </td>

                            <td>
                              <div className='overflow-hidden text-ellipsis line-clamp-1'>
                                {isZNonEmptyString(el?.name)
                                  ? el?.name
                                  : constants.noValueFound}
                              </div>
                            </td>
                            ...
                            <td>
                              <div className='overflow-hidden ms-4 text-ellipsis line-clamp-1'>
                                {el?.invoices_count || constants.noValueFound}
                              </div>
                            </td>
                            ...
                            <td>
                              <div className='overflow-hidden text-ellipsis line-clamp-1'>
                                {isZNonEmptyString(
                                  String(el?.default_currency?.value)
                                )
                                  ...
                                  onClick={() => {
                                    void (async () => {
                                      const { value } = await showZPrompt({
                                        title:
                                          messages.client.confirmDialog.title,
                                        message:
                  ...
                  <div className='flex justify-center mt-5 items-center w-full py-[.4rem]'>
                    <div className='w-1/2'>
                      <span className='font-normal ps-4 text-primary text-[1rem] leading-[120%] font-roboto-medium'>
                        ...
                        {zClientsDataRStateSelector?.data?.length} results
                      </span>
                    </div>
