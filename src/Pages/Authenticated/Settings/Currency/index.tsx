        ...
        onSubmit={(values, { setFieldError }) => {
          void (async () => {
            ...
            if (values.logoFile !== null && values.logoFile !== undefined) {
              ...
              const _fileUploadRes = await UploadFileHandler(formData);

              if (_fileUploadRes) {
                _filePath = _fileUploadRes?.filePath;
                _fileUrl = _fileUploadRes?.fileUrl;
              }
            }
            const _reqData: {
              default_currency: string;
            ...
            if (_filePath && _fileUrl) {
              _reqData['logo'] = zStringify({
                path: _filePath,
                url: _fileUrl
              });
            }

            const zStringifyData = zStringify(_reqData);
